...
public class OverzichtFrame extends JFrame implements ActionListener {
	...
	public OverzichtFrame(Bedrijf b, HoofdFrame hf) {
		bedrijf = b;
		hoofdFrame = hf;
		...
		pack(); setTitle("Overzicht reserveringen");
			...
			if(!ejVeld.getText().matches("((-|\\+)?[0-9]+(\\.[0-9]+)?)+") || Integer.parseInt(ejVeld.getText()) <= 2013 || Integer.parseInt(ejVeld.getText()) > 5000){
				...
				try{
					...
					if(begDat.after(eindDat)){
						...
						String s ="Reservering binnen de periode: " + df.format(begDat) + " tot " + df.format(eindDat);
						//Alle reserveringen die binnen de ingegeven data zitten worden getoond
